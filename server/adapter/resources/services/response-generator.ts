...
export class ResponseGenerator {
  static generate(schemaResponse: any, preferredExampleName?: any) {
    if (
      schemaResponse.example ||
      (schemaResponse.examples && Object.values(schemaResponse.examples).length)
    ) {
      const bestExample = this.getBestExample(
        schemaResponse,
        preferredExampleName
      );
      if (bestExample !== undefined) return bestExample;
    }
  ...
  static getBestExample(schemaResponse: any, preferredExampleName: any) {
    ...
    if ((Object.values(schemaResponse.examples)[0] as any).value)
      return (Object.values(schemaResponse.examples)[0] as any).value;
  }
  ...
  static generateByType(schemaResponse: any) {
  ...
  static generateByFaker(fakerString) {
    ...
    if (fakerString.match(/\{\{.+\}\}/)) return faker.fake(fakerString);

    const fakerRegex =
      /^(?<namespace>\w+)\.(?<method>\w+)(?:\((?<argsString>.*)\))?$/.exec(
        fakerString
      );

    if (!fakerRegex)
      throw new Error(
        'Faker extension method is not in the right format. Expecting <namespace>.<method> or <namespace>.<method>(<json-args>) format.'
      );
